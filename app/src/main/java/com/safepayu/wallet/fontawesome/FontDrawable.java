package com.safepayu.wallet.fontawesome;
...
import com.safepayu.wallet.custom_view.FontCache;
...
public class FontDrawable extends Drawable {
    ...
    private void init(Context context, int faIconRes, boolean isSolid, boolean isBrand) {
        ...
        Typeface tf = null;
        if (isSolid) {
            tf = FontCache.getTypeface(FontCache.FA_FONT_SOLID, context);
        } else if (isBrand) {
            tf = FontCache.getTypeface(FontCache.FA_FONT_BRANDS, context);
        } else {
            tf = FontCache.getTypeface(FontCache.FA_FONT_REGULAR, context);
        }
