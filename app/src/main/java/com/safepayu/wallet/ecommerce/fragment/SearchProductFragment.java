...
public class SearchProductFragment extends Fragment implements View.OnClickListener, SearchProductAdapter.OnProductDetailItemListener,
    ...
    public void onProductItemDetail(int position,ProductsByCategoryIdResponse.ProductsBean productsBean) {
        ...
        BaseApp.getInstance().sharedPref().setString(BaseApp.getInstance().sharedPref().PRODUCT_ID,String.valueOf(productsBean.getId()));
        if (Integer.parseInt(productsBean.getOffer_id())!=0)
        { BaseApp.getInstance().sharedPref().setString(BaseApp.getInstance().sharedPref().OFFER_ID,String.valueOf(productsBean.getId())); }
                        ...
                        if (response.isStatus()) {
                            ...
                            if (response.getCategory().size()>0){
                                ...
                                if (response.getProducts().size()>0){
                                    ...
                                    responseSort.getProducts().clear();
